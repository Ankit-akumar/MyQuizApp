...
class QuizAdapter(private val mQuizzes: List<Quiz>) :
    ...
    override fun onBindViewHolder(holder: QuizAdapter.ViewHolder, position: Int) {
        val quiz: Quiz = mQuizzes[position]
        val textView = holder.titleTextView
        textView.text = quiz.name
        holder.descTextView.text = quiz.desc
        holder.backgroundImageView.setImageResource(quiz.image)
    }
